...
module.exports = class ECDSA_KEY {
	 ...
	 *
	 * @param {Object} key This must be the "privKeyObj" or "pubKeyObj" part of the object generated by sm2.KEYUTIL.generateKeypair()
	 */
	constructor(key) {
		...
		if (!key.type || key.type !== 'EC') {
			throw new Error('This key implementation only supports keys generated by sm2.KEYUTIL. It must have a "type" property of value "EC"');
		}

		// prvKeyHex value can be null for public keys, so need to check typeof here
		if (typeof key.prvKeyHex === 'undefined') {
			throw new Error('This key implementation only supports keys generated by sm2.KEYUTIL. It must have a "prvKeyHex" property');
		}

		// pubKeyHex must have a non-null value
		if (!key.pubKeyHex) {
			throw new Error('This key implementation only supports keys generated by sm2.KEYUTIL. It must have a "pubKeyHex" property');
		}
